...
int FindPlates(Mat &imageSource, vector<Rect> &rectsNotRotated, vector<RotatedRect> &rectsRotated, Mat &imageDebug, int iDebugMode) {
    ...
    Mat imageMorph2;                    // Thin horiz lines removed
    Mat imageMorph3;                    // Thin vert lines removed
    ...
    Mat elementRemoveThinHorizLines = getStructuringElement(MORPH_RECT, Size(1,10));
    Mat elementRemoveThinVertLines = getStructuringElement(MORPH_RECT, Size(3,1));
    ...
    morphologyEx(imageMorph, imageMorph2, CV_MOP_OPEN, elementRemoveThinHorizLines);

    if (iDebugMode == AVS_DEBUG_MODE_DISPLAY_MORPH2) {
    	// Need to convert it to colour before we can copy it
    	cvtColor(imageMorph2, imageDebug, CV_GRAY2BGRA);  // Converts and copies in one go
        }

#ifdef AVS_QT
    imshow("Morph2", imageMorph2);
#endif // AVS_QT

    morphologyEx(imageMorph2, imageMorph3, CV_MOP_OPEN, elementRemoveThinVertLines);

    if (iDebugMode == AVS_DEBUG_MODE_DISPLAY_MORPH2) {
    	// Need to convert it to colour before we can copy it
    	cvtColor(imageMorph3, imageDebug, CV_GRAY2BGRA);  // Converts and copies in one go
        }

    findContours(imageMorph3, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
...
bool verifySizes(RotatedRect rectCandidatePlate) {
    ...
    if (rectBounding.width < 30 || rectBounding.height < 20) {
...
void FindBestPlateCandidate(vector<RotatedRect> &rectsRotated, RotatedRect &rotatedrectBestPlate, int iImageWidth, int iImageHeight) {
    ...
    double fXScore, fYScore, fFlatness;
    double fRotatedRectHeight, fRotatedRectWidth; // OpenCV confuses height and width, so I have to find out which is which then store it
    double fAspectRatioScore;
    double fOverallScore;
    double fBestScore;
    ...
    for (size_t i = 0; i < rectsRotated.size(); i++) {
        ...
        printf("Scoring plate candidate\n");
        printf("-----------------------\n");

        //
        // Score X-centre position (from 1 if it's exactly image centre, to 0 if it's on the very edge)
        //

        fXScore = 1.0 - (fabs((rectangle.x + (rectangle.width/2)) - (iImageWidth / 2)) / (iImageWidth / 2));

        // Same for Y-centre

        fYScore = 1.0 - (fabs((rectangle.y + (rectangle.height/2)) - (iImageHeight / 2)) / (iImageHeight / 2));

        printf("fXScore = %6.4f\n", fXScore);
        printf("fYScore = %6.4f\n", fYScore);

        // OpenCV's rotated rect structure uses 'height' and 'width' pretty randomly - so we
        // need to find out which is which.
        if (rotatedrectTemp.size.height < rotatedrectTemp.size.width) {
            fRotatedRectHeight = rotatedrectTemp.size.height;
            fRotatedRectWidth = rotatedrectTemp.size.width;
            }
        else {
            fRotatedRectHeight = rotatedrectTemp.size.width;
            fRotatedRectWidth = rotatedrectTemp.size.height;
            }

        fFlatness = fRotatedRectHeight / rectangle.height;

        printf("fFlatness = %6.4f\n", fFlatness);

        printf("Aspect ratio = %6.4f\n", fRotatedRectHeight / fRotatedRectWidth);
        //fAspectRatioScore = 1.0 - (fabs((fRotatedRectHeight / fRotatedRectWidth) - PLATE_ASPECT_RATIO) * 5.0);
        fAspectRatioScore = 1.0 - ((fabs((fRotatedRectHeight / fRotatedRectWidth) - PLATE_ASPECT_RATIO) * 5.0));
        printf("Aspect ratio score = %6.4f\n", fAspectRatioScore);

        //
        // With all that information, try to decide the overall score for this plate candidate.
        //

        fOverallScore = (fXScore + fYScore + fFlatness) * fAspectRatioScore;

        printf("Overall score: %6.4f\n", fOverallScore);

        // Store the index position of the best rectangle found
        if (fOverallScore > fBestScore) {
            fBestScore = fOverallScore;
            max_rect_idx = i;
            }
        } // End 'for each rectangle'

    rotatedrectBestPlate = rectsRotated[max_rect_idx];
